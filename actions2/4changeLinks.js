...
const landingTexts = {
    "ru-jur": { h: "Подтвердите, что вы не робот", b: "Я не робот", url: "https://za.zavod93.ru/D4QMKY", code: "jur-ru" },
    "it-18": { h: "Contenuti per persone con più di 18 anni", b: "Ho più di 18 anni", url: "https://za.zavod93.ru/G7xp4s9P", code: "ad-it" },
};

let lText = landingTexts["ru-jur"];
...
function main() {
    ...
    if (source === "--one") {
        // console.log();
        const textsKeys = Object.keys(landingTexts);
        console.log("Select text:");
        for (let iT in textsKeys) {
            console.log(`${iT}) ${textsKeys[iT]}`);
        }
        let textVariant = -1;
        while (textVariant < 0 || textVariant >= textsKeys.length) {
            textVariant = prompt("Type number (0 or 1 or 2 ...) (exit - to exit): ");
            if (textVariant === "exit") {
                console.log("user escaped");
                return;
            }
        }
        lText = landingTexts[textsKeys[textVariant]];
        USER_NAME = args[3];
        // DOMAIN_ZONE = args[4];
        ...
        // console.log("businessNames", businessNames);
        // console.log("DOMAIN_ZONE", DOMAIN_ZONE);
        console.log("data_1_label", data_1_label, "<- not required. set up in jsons");
        console.log("user_label", user_label, "<- not required. set up in jsons");
        console.log(" ================================== ");
        const confirm = prompt("Continue? ( y ): ");
        if (confirm !== "y") {
            console.log("user escaped");
    ...
    puppeteer
        ...
        .then(async (browser) => {
            ...
            for (let ch in changableArray) {
                ...
                const dataLabel1 = source === "--one" ? data_1_label : entity.data_1_label;
                const userLabel = source === "--one" ? user_label : entity.user_label;

                const baseUrl = lText.url + "?chan=";
...
async function changeFunction(browser, entity) {
    ...
    const { baseUrl, userLabel, dataLabel1, dataLabel2, filename, folderPath, lText } = entity;
    const tail1 = `&utm_term={keyword}&utm_creative={creative}&utm_position={adposition}&utm_network={network}&utm_target={target}&utm_placement={placement}&utm_match={matchtype}&xutm_content={placement}&utm_content={placement}&utm_source=nn&utm_medium={creative}`;
    const tail2 = `${tail1}&utm_campaign=${userLabel}-${dataLabel1}-${dataLabel2}-${lText.code}`;
    entity.url = `${baseUrl}${userLabel}&data1=${dataLabel1}&data2=${dataLabel2}${tail2}`;
    console.log("ENTITY URL = ", entity.url);
    const busFileJson = fs.readFileSync(`${folderPath}/godaddySites/${filename}.json`, "utf8");
...
async function changeFooter(page, entity) {
    ...
    try {
        ...
        const footer = await page.waitForSelector("div[data-aid*='WIDGET_WRAPPER_FOOTER']");
        const textElement = await footer.$(".DraftEditor-root");
        await textElement.click();
        await page.waitForTimeout(tM * 1000);

        const muttator = await page.waitForSelector("div[data-aid='MUTATOR_PAGE_NAVIGATION']");
        await page.waitForTimeout(tM * 1000);
        ...
        const subStringInpt = await page.waitForSelector("input[widgettype='FOOTER']");
        ...
        const confirmOkFooterChangeBtn = await page.waitForSelector("button[data-aid='EDITOR_HEADER_DONE_BUTTON']");
...
async function changeHeader(page, entity) {
    ...
    try {
        await page.goto(entity.editUrl);
        await page.waitForTimeout(tM * 5000);
        const header = await page.waitForSelector("div[data-aid*='WIDGET_WRAPPER_HEADER']");
        header.click();
        await page.waitForTimeout(tM * 1000);
        ...
        await textArea1.evaluate((el, lText) => {
            el.innerText = lText.h;
            el.value = lText.h;
        }, lText);
        ...
        await page.waitForTimeout(tM * 1000);
        const confirmOkLogoChangeBtn = await page.waitForSelector("button[data-aid='EDITOR_HEADER_DONE_BUTTON']");
        ...
        await page.waitForTimeout(tM * 1000);
        const confirmOkBtnChangeBtn = await page.waitForSelector("button[data-aid='EDITOR_HEADER_DONE_BUTTON']");
        ...
        header.click();
        await page.waitForTimeout(tM * 1000);
        const styleBtn = await page.waitForSelector("div[data-aid='CHANGE_STYLE_PIVOT']");
        ...
        const insertBtn = await page.waitForSelector("button[data-aid='insert_images']");
        console.log("clicking insert button ...");
        // console.log(insertBtn);
        await insertBtn.click();
        await page.waitForTimeout(tM * 1000);

        const confirmOkImageChangeBtn = await page.waitForSelector("button[data-aid='EDITOR_HEADER_DONE_BUTTON']");
        await confirmOkImageChangeBtn.click();
        await page.waitForTimeout(tM * 1000);

        const switchViewBtn = await page.waitForSelector("div[data-aid='SITE_TABS_WEBSITE']");
...
async function deleteBlocks(page, entity) {
    ...
    try {
        ...
        for (;;) {
            ...
            try {
                const renderContainer = await page.waitForSelector("div#render-container");
                const blocks = await renderContainer.$$("div[data-aid*='WIDGET_WRAPPER_']");
                ...
                for (let i in blocks) {
                    ...
                    const dataAid = await blocks[i].evaluate((el) => el.getAttribute("data-aid"));
                    console.log(dataAid);
                ...
                if (blocks.length > skippedBlock) {
                    ...
                    if (hCandidate) {
                        ...
                        await page.waitForTimeout(tM * 1000);
                        const editBlockBtn = await page.waitForSelector("div[data-aid='SECTION_MANAGER_SETTINGS']");
                        for (;;) {
                            try {
                                ...
                                const deleteBtn = await page.waitForSelector("div[data-aid='DELETE_WIDGET_FLYOUT_OPTION']");
                                await deleteBtn.click();
                                break;
                            } catch (error) {
                                console.error(error);
                                console.log("Cannot click DELETE_WIDGET_FLYOUT_OPTION");
                            }
                        }

                        await page.waitForTimeout(tM * 1000);
                        const confirmDeleteBtn = await page.waitForSelector("button[data-aid='DELETE_WIDGET_MODAL_DELETE']");
...
async function deletePages(page, entity) {
    ...
    try {
        ...
        for (;;) {
            ...
            await item.hover();
            await page.waitForTimeout(tM * 1000);
            let itemSettings = await item.$("div[data-aid='PAGE_SETTINGS_COG_ICON']");
            await itemSettings.click();

            try {
                await page.waitForTimeout(tM * 1000);
                const settingMenu = await page.waitForSelector("div[data-aid='PAGE_KEBAB_MENU']");
                // DELETE_PAGE_BUTTON
                const deleteBtn = await settingMenu.$("div[data-aid='DELETE_PAGE_BUTTON']");
                if (!deleteBtn) {
                ...
                const confirmDeleteBtn = await page.waitForSelector("button[data-aid='DELETE_PAGE_MODAL_DELETE']");
                await confirmDeleteBtn.click();
                await page.waitForTimeout(tM * 1000);
            } catch (error) {
...
async function changeTheme(page, entity) {
    ...
    try {
        ...
        const themeTabBtn = await page.waitForSelector("div[data-aid='SITE_TABS_THEME'");
        ...
        const changeThemeBtn = await page.waitForSelector("button[data-aid='THEME_CUSTOMIZER_TRY_NEW_LOOK_BUTTON']");
        await changeThemeBtn.click();
        console.log("Select theme  ....");

        const theme = await page.waitForSelector("div[data-aid='THEME_GALLERY_THUMBNAIL_MODERN']");
        theme.click();
        await page.waitForTimeout(tM * 1000);

        const colorAccordion = await page.waitForSelector("div[data-aid='THEME_ACCORDION_COLOR']");
...
async function turnOffMsg(page, entity) {
    ...
    try {
        ...
        const messageBtn = await page.waitForSelector("div[data-aid='ADDON_BUTTON_MESSAGING']");
        await messageBtn.click();
        const checkboxMail = await page.waitForSelector("input[type='checkbox']");
        await checkboxMail.click();

        const mailChangesSubmitBtn = await page.waitForSelector("button[data-aid='EDITOR_HEADER_DONE_BUTTON']");
        await mailChangesSubmitBtn.click();
        await page.waitForTimeout(tM * 5000);

        // addKeyToBusiness(filename, key, true);
        entity.changeHasBeenMade = true;
...
function generateColor() {
    const primaryColors = ["A", "B", "C", "D", "E", "F", "A", "B", "C", "D", "E", "F"];
...
function addKeyToBusiness(folderPath, filename, key, value = true) {
    try {
        ...
        fs.writeFileSync(`${folderPath}/godaddySites/${filename}.json`, __bus__);
        return 1;
    } catch (error) {
        console.log(error);
