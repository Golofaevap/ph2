...
async function main() {
    ...
    const page1 = await newPage(browser, opts);
    await page1.goto(session.workFakeEmail);
    ...
    await page2.goto("https://pornhub.com");
    await page2.reload({ waitUntil: ["networkidle0", "domcontentloaded"] });

    // get rid of age banner
    try {
        await page2.setDefaultTimeout(1000);
        await page2.setDefaultNavigationTimeout(2000);
        await page2.waitForTimeout(5000);
        const ageButtonDiv = await page2.waitForSelector("div[class='contentMTubes']");
        const ageButton = await ageButtonDiv.$("button");
        const btnText = await ageButton.evaluate((el) => {
            return el.innerText;
        });
        console.log(btnText);
        if (btnText && btnText.includes("18")) {
    ...
    session.phRegistered = false;
    session.phEmailVerified = false;
    ...
    await page2.setDefaultTimeout(100000);
    await page2.setDefaultNavigationTimeout(200000);
    // click signUp on main page
    await page2.waitForTimeout(3000);
    const signUp = await page2.waitForSelector("a#headerSignupLink");
    await signUp.click();

    // click sign up for free
    await page2.waitForTimeout(3000);
    const singUpBtnsDiv = await page2.waitForSelector("div[class='buttons']");
    console.log(
        await singUpBtnsDiv.evaluate((el) => {
            return el.innerText;
        })
    );
    const freeSignUpBtn = await singUpBtnsDiv.$("a[class='btnGrey']");
    await freeSignUpBtn.click();

    // fill the form
    await page2.waitForTimeout(3000);
    const formDiv = await page2.waitForSelector("div[class='formWrapper']");
    const inputEmail = await formDiv.$("input[name='email']");
    await inputEmail.type(session.mail, { delay: 40 });

    await page2.waitForTimeout(3000);
    const inputNik = await formDiv.$("input[name='username']");
    await inputNik.click({ clickCount: 3 });
