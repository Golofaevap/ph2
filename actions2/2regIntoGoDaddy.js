...
function main() {
    ...
    if (!args[3]) {
        return console.log(
            "Please setup domain zone".red,
            "email:",
            args[3],
            "can be: ru-RU, en-US, en-GB"
            /*, "pwds:",
          args[3]*/
        );
    }
    const USER_NAME = args[2];
    const DOMAIN_ZONE = args[3];
    if (
        !(
            DOMAIN_ZONE === "en-US" ||
            DOMAIN_ZONE === "ru-RU" ||
    ...
    if (isGoDaddy) {
        console.log(
            "GoDaddy has already been created for this user...".bgYellow.blue
        );
    ...
    puppeteer
        ...
        .then(async (browser) => {
            ...
            try {
                ...
                await page.goto(
                    `https://sso.godaddy.com/v1/account/create?realm=idp&app=dashboard.api&path=%2Fvh-login-redirect&marketid=${DOMAIN_ZONE}&iframe=false`
                );
                // await page.wait()
                // try {
